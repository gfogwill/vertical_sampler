...
def main():
    ...
    # Keep the unpopulated OPC chip-select inactive on the shared SPI bus.
    shared_spi = SharedSPI(spi, config.OPC_CS, config.LORA_CS)
    lora = LoRa(
        spi,
        config.CARLA_RFM_ADDRESS,
        config.GROUND_RFM_ADDRESS,
        shared_spi=shared_spi,
    )
